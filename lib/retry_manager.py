...
class RetryManager:
    def __init__(self, 
                 ...
                 recovery_file: str = "output/recovery_data.json"):
        ...
        for attempt_num in range(1, self.max_retries + 1):
            ...
            try:
                ...
                if attempt_num == 1:
                    ...
                    print(f"   Tool: {tool_name}")
                    print(f"   Patent: {patent_id}")
                    print(f"   Attempt: {attempt_num}/{self.max_retries}")
                ...
                if result and not result.startswith("ERROR IN") and not result.startswith("SKIPPED"):
                    ...
                    # Also print success details to console
                    if attempt_num > 1:
                        print(f"\n🟢 RETRY SUCCESS:")
                        print(f"   Tool: {tool_name}")
                        print(f"   Patent: {patent_id}")
                        print(f"   Succeeded on attempt: {attempt_num}/{self.max_retries}")
                        print(f"   Duration: {duration:.2f}s")
                        print(f"   Previous failures: {attempt_num - 1}")
                ...
                # Log error details more prominently
                ...
                if attempt_num == self.max_retries:
                    ...
                    # Print comprehensive failure summary to console
                    print(f"\n🔴 TOOL COMPLETELY FAILED:")
                    print(f"   Tool: {tool_name}")
                    print(f"   Patent: {patent_id}")
                    print(f"   Max retries reached: {self.max_retries}")
                    print(f"   Final error: {error_type}: {error_message}")
                    print(f"   Total duration: {sum(a.duration for a in record.attempts):.2f}s")
                    print(f"   All attempts:")
                    for i, attempt in enumerate(record.attempts, 1):
                        print(f"     {i}. {attempt.error_type}: {attempt.error_message[:100]}...")
                    print(f"   Generating fallback result...")
